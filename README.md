...
```
GOOGLE_APPLICATION_CREDENTIALS=/app/env/firebase-service-account.json
```

서버에 생성되는 디렉토리 구조:
```
/home/woopi/project/pigeon/
├── env/
│   ├── firebase-service-account.json   ← FIREBASE_SERVICE_ACCOUNT_JSON 시크릿으로 자동 생성
│   └── .env                            ← DB/Sentry 환경변수 자동 생성
└── docker-compose.yml
```
